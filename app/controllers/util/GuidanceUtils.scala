...
object GuidanceUtils {
  ...
  sealed trait GuidanceColumnWriter {
    ...
    def value(keyToTdrHeader: String => String, keyToPropertyType: String => String): MetadataProperty => Any
    def systemPropertyFormatting(guidanceItem: MetadataProperty): StyleSetter => Unit = ss =>
      if (guidanceItem.format == "Do not modify")
        ss.fontColor(SYSTEM_ROW_FONT_COLOR).set()
      else ss.fontColor(DEFAULT_FONT_COLOR).set()
    def formatDataType(keyToPropertyType: String => String, guidanceItem: MetadataProperty): StyleSetter => Unit = _ => ()
  }
  ...
  case object ColumnTitle extends GuidanceColumnWriter {
    ...
    def value(keyToTdrHeader: String => String, keyToPropertyType: String => String): MetadataProperty => Any = gi => keyToTdrHeader(gi.property)
  ...
  case object Details extends GuidanceColumnWriter {
    ...
    def value(keyToTdrHeader: String => String, keyToPropertyType: String => String): MetadataProperty => Any = _.details
  ...
  case object Format extends GuidanceColumnWriter {
    ...
    def value(keyToTdrHeader: String => String, keyToPropertyType: String => String): MetadataProperty => Any = _.format
    override def columnLevelFormatting: StyleSetter => Unit = _.wrapText(true).set()
  }

  case object TDRRequirement extends GuidanceColumnWriter {
    val header = "TDR REQUIREMENT"
    def value(keyToTdrHeader: String => String, keyToPropertyType: String => String): MetadataProperty => Any = _.tdrRequirement
  }
  ...
  case object Example extends GuidanceColumnWriter {
    ...
    override def formatDataType(keyToPropertyType: String => String, guidanceItem: MetadataProperty): StyleSetter => Unit = { ss =>
