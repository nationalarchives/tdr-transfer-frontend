...
class AdditionalMetadataNavigationController @Inject()(val consignmentService: ConsignmentService,
  ...
  def getAllFiles(consignmentId: UUID, metadataType: String): Action[AnyContent] = standardTypeAction(consignmentId) { implicit request: Request[AnyContent] =>
    consignmentService.getAllConsignmentFiles(consignmentId, request.token.bearerAccessToken).map(allFiles => {
  ...
  def submitFiles(consignmentId: UUID, metadataType: String): Action[AnyContent] = standardTypeAction(consignmentId) { implicit request: Request[AnyContent] =>
    val fileIds = request.body.asFormUrlEncoded.getOrElse(Map()).keys.toList
      .filter(_ != "csrfToken")
      .map(UUID.fromString)
