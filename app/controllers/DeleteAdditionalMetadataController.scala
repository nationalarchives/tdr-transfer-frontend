        ...
        for {
          ...
          response <-
            if (consignment.files.nonEmpty) {
              ...
              val hasEnteredMetadata = consignment.files.flatMap(_.fileStatuses.filter(_.statusType.contains(metadataType.capitalize))).exists(_.statusValue != Statuses.NotEnteredValue.value)
              Future(
