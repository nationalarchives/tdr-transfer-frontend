...
class UploadController @Inject()(val controllerComponents: SecurityComponents,
  ...
  def driveSubmit(consignmentId: UUID) : Action[AnyContent] = standardTypeAction(consignmentId) { implicit request: Request[AnyContent] =>
    for {
      ...
      clientMetadataInput = AddFileAndMetadataInput(consignmentId, fileMetadata.zipWithIndex.map{ case (m,i) =>
      ...
      transferEvent = clientMetadata.map(i =>
        DriveTransferEvent(token, "tdr-upload-files-cloudfront-dirty-intg", s"/${parentFolder._1}", grouped(i.matchId).originalPath, s"${request.token.userId}/$consignmentId/${i.fileId}")
      )
