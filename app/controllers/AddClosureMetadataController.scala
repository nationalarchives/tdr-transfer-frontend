...
class AddClosureMetadataController @Inject() (
  ...
  def addClosureMetadataSubmit(consignmentId: UUID, fileIds: List[UUID]): Action[AnyContent] = standardTypeAction(consignmentId) { implicit request: Request[AnyContent] =>
    for {
