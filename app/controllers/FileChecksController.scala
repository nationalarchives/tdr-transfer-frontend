...
class FileChecksController @Inject() (
  ...
  def backendChecks(consignmentId: UUID, token: BearerAccessToken): Future[Boolean] = {
    for {
      ...
      uploadStatusUpdate = if (backendChecksTriggered) { CompletedValue } else { CompletedWithIssuesValue }
  ...
  def fileChecksPage(consignmentId: UUID): Action[AnyContent] = standardTypeAction(consignmentId) { implicit request: Request[AnyContent] =>
    ...
    for {
      ...
      fileChecks <- if (backendChecksTriggered) { getFileChecksProgress(request, consignmentId) } else { throw new Exception(s"Backend checks trigger failure for consignment $consignmentId") }
