...
import controllers.util.TransferProgressUtils.{
  ClientChecksCompleted,
  ClientChecksFailed,
  ClientChecksInProgress,
  ConfirmTransferCompleted,
  ExportCompleted,
  ExportFailed,
  ExportInProgress,
  FileChecksCompleted,
  FileChecksFailed,
  FileChecksInProgress,
  SeriesCompleted,
  SeriesInProgress,
  TransferAgreementCompleted,
  TransferAgreementInProgress,
  UploadCompleted,
  UploadFailed,
  UploadInProgress
}
...
class ViewTransfersControllerV2 @Inject() (
    val consignmentService: ConsignmentService,
    val transferProgressUtils: TransferProgressUtils,
  ...
  def standardStatusesToUserAction(statuses: CurrentStatus, consignmentId: UUID, consignmentRef: String, consignmentType: String): UserAction = {
    transferProgressUtils.toTransferState(statuses) match {
      ...
      case UploadInProgress | UploadCompleted => UserAction(transferStatusInProgress, routes.UploadController.uploadPage(consignmentId).url, resumeTransfer)
      case UploadFailed                       => UserAction(transferStatusFailed, routes.UploadController.uploadPage(consignmentId).url, resumeTransfer)
      case ClientChecksInProgress | ClientChecksCompleted | FileChecksInProgress =>
        UserAction(transferStatusInProgress, routes.FileChecksController.fileChecksPage(consignmentId).url, resumeTransfer)
      case ClientChecksFailed                 => UserAction(transferStatusFailed, routes.FileChecksController.fileChecksPage(consignmentId).url, viewErrors)
      case FileChecksCompleted                => UserAction(transferStatusInProgress, routes.FileChecksResultsController.fileCheckResultsPage(consignmentId).url, resumeTransfer)
      case FileChecksFailed                   => UserAction(transferStatusFailed, routes.FileChecksResultsController.fileCheckResultsPage(consignmentId).url, viewErrors)
      case ConfirmTransferCompleted           => UserAction(transferStatusInProgress, routes.FileChecksResultsController.fileCheckResultsPage(consignmentId).url, resumeTransfer)
      case ExportInProgress | ExportCompleted => UserAction(transferStatusInProgress, routes.DownloadMetadataController.downloadMetadataCsv(consignmentId).url, downloadReport)
...
case class ConsignmentTransfersV2(
...
case class UserActionV2(transferStatus: String, actionUrl: String, actionText: String)
