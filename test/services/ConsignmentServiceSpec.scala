  ...
  "getConsignmentFileMetadata" should {
    "return consignment with file metadata when consignment id and selected file ids are passed" in {
      ...
      val graphQlGetConsignmentFilesMetadata =
        gcfm.GetConsignment(List(gcfm.GetConsignment.Files(fileId, Nil, gcfm.GetConsignment.Files.Metadata(exemptionCode, None, None, None, None))), "TEST-TDR-2021-GB")
  ...
  "getConsignmentPaginatedFile" should {
    "return paginated file information for a consignment given a folderId" in {
      ...
      val paginatedFiles: gcpf.GetConsignment.PaginatedFiles =
        PaginatedFiles(
          PageInfo(startCursor = None, endCursor = None, hasNextPage = true, hasPreviousPage = false),
          Some(
            List(
      ...
      when(
        getConsignmentPaginatedFilesClient.getResult(
          bearerAccessToken,
          gcpf.document,
          Some(gcpf.Variables(consignmentId, Some(PaginationInput(limit, currentPage = Some(page), currentCursor = None, Some(FileFilters(None, None, Some(folderId)))))))
        )
      ).thenReturn(Future.successful(response))
    ...
    "return an error if there is an error from the API" in {
      ...
      val results = consignmentService.getConsignmentPaginatedFile(consignmentId, page = page, limit = limit, selectedFolderId = folderId, bearerAccessToken)
      results.failed.futureValue shouldBe a[HttpError]
    }
  }
