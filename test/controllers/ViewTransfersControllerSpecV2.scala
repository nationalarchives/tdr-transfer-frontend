...
class ViewTransfersControllerSpecV2 extends FrontEndTestHelper {
  ...
  val transferStateToAction: Map[TransferState, (String, String, String)] = Map(
    ...
    ExportFailed -> ("mailto:%s?subject=Ref: TEST-TDR-2023-X - Export failure", "In Progress", "Download report")
  )
...
//      val mockTransferProgressUtils = mock[TransferProgressUtils]
//      val consignmentId = UUID.randomUUID()
//      val controller = new ViewTransfersController(consignmentService, mockTransferProgressUtils, getValidStandardUserKeycloakConfiguration, getAuthorisedSecurityComponents)
//
//      ConsignmentStatusesOptions.expectedStandardStatesAndStatuses.map(s => {
//        val transferState = s._1
//        when(mockTransferProgressUtils.toTransferState(any[CurrentStatus])).thenReturn(transferState)
//        val userAction = controller.standardStatusesToUserAction(CurrentStatus(s._2, s._3, s._4, s._5, s._6, s._7, s._8, s._9, s._10), consignmentId, "TEST-TDR-2023-X", "standard")
//        val expectedValues = transferStateToAction(transferState)
//        userAction.actionUrl mustBe s"/consignment/$consignmentId${expectedValues._1}"
//        userAction.transferStatus mustBe expectedValues._2
//        userAction.actionText mustBe expectedValues._3
//      })
//    }
    ...
    "render the view transfers page with list of user's consignments" in {
      ...
      val consignmentService = new ConsignmentService(graphQLConfiguration)
      val mockTransferProgressUtils = mock[TransferProgressUtils]
      val controller = new ViewTransfersControllerV2(consignmentService, mockTransferProgressUtils, getValidStandardUserKeycloakConfiguration, getAuthorisedSecurityComponents)
    ...
    "render the view transfers page with list of judgment user's consignments" in {
      ...
      val consignmentService = new ConsignmentService(graphQLConfiguration)
      val mockTransferProgressUtils = mock[TransferProgressUtils]
      val controller = new ViewTransfersControllerV2(consignmentService, mockTransferProgressUtils, getValidJudgmentUserKeycloakConfiguration, getAuthorisedSecurityComponents)
      val response = controller
        .viewConsignments()
        .apply(FakeRequest(GET, s"/view-transfers"))
      val viewTransfersPageAsString = contentAsString(response)
    ...
    "redirect to the login page if the page is accessed by a logged out user" in {
      ...
      val consignmentService = new ConsignmentService(graphQLConfiguration)
      val mockTransferProgressUtils = mock[TransferProgressUtils]
      val controller = new ViewTransfersControllerV2(consignmentService, mockTransferProgressUtils, getValidStandardUserKeycloakConfiguration, getUnauthorisedSecurityComponents)
      val response = controller
        .viewConsignments()
        .apply(FakeRequest(GET, s"/view-transfers"))
