...
class HomepageControllerSpec extends FrontEndTestHelper {
  ...
  "HomepageController GET" should {
    ...
    "render the homepage page with an authenticated user with no user type" in {
      ...
      val homepage = controller.homepage().apply(FakeRequest(GET, "/homepage"))
      status(homepage) mustBe OK
      contentType(homepage) mustBe Some("text/html")
      contentAsString(homepage) must include ("Welcome")
      contentAsString(homepage) must include ("Welcome to the Transfer Digital Records service")
      contentAsString(homepage) must include ("Upload your records to start a new transfer")
    }
    ...
    "render the judgment homepage page with an authenticated judgment user" in {
      val controller = new HomepageController(
        ...
        consignmentService)
      val homepage = controller.homepage().apply(FakeRequest(GET, "/homepage").withCSRFToken)
      status(homepage) mustBe OK
      contentType(homepage) mustBe Some("text/html")

      contentAsString(homepage) must include ("Welcome")
      contentAsString(homepage) must include ("Welcome to the Transfer Digital Records service")
      contentAsString(homepage) must include ("Upload your judgment to start a new transfer")
    }
    ...
    "return a redirect to the auth server with an unauthenticated user" in {
      val controller = new HomepageController(
        ...
        consignmentService)
      val homepage = controller.homepage().apply(FakeRequest(GET, "/homepage"))
      redirectLocation(homepage).get must startWith("/auth/realms/tdr/protocol/openid-connect/auth")
      status(homepage) mustBe FOUND
    }
  }
  ...
  "HomepageController POST" should {
    ...
    "return a redirect to the auth server with an unauthenticated user" in {
      val controller = new HomepageController(
        ...
        getValidKeycloakConfiguration,
        consignmentService)
      val homepage = controller.homepage().apply(FakeRequest(POST, "/homepage").withCSRFToken)
      redirectLocation(homepage).get must startWith("/auth/realms/tdr/protocol/openid-connect/auth")
      status(homepage) mustBe SEE_OTHER
    }
  }
}
