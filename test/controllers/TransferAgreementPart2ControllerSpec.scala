...
import testUtils.DefaultMockFormOptions.expectedPart2Options
...
class TransferAgreementPart2ControllerSpec extends FrontEndTestHelper {
  ...
  s"TransferAgreementPart2Controller GET" should {
    val inputOptions = expectedPart2Options
    ...
    "return a redirect to the auth server with an unauthenticated user" in {
      val consignmentId = UUID.fromString("c2efd3e6-6664-4582-8c28-dcf891f60e68")
      val controller: TransferAgreementPart2Controller =
        taHelper.instantiateTransferAgreementPart2Controller(getUnauthorisedSecurityComponents, app.configuration)
    ...
    "create a transfer agreement (part 2) when a valid form is submitted and the api response is successful" in {
      ...
      taHelper.stubTAPart2Response(Some(addTransferAgreementResponse), app.configuration)
    ...
    "render an error when a valid (part2) form is submitted but there is an error from the api" in {
      val consignmentId = UUID.fromString("c2efd3e6-6664-4582-8c28-dcf891f60e68")
      taHelper.stubTAPart2Response(config = app.configuration, errors = List(GraphQLClient.Error("Error", Nil, Nil, None)))
