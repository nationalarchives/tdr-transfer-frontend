...
class SeriesDetailsControllerSpec extends FrontEndTestHelper {
  ...
  "SeriesDetailsController GET" should {
    ...
    "render the series details page with an authenticated user" in {
      val controller = new SeriesDetailsController(getAuthorisedSecurityComponents(), new GraphQLConfiguration(app.configuration))
    ...
    "return a redirect to the auth server with an unauthenticated user" in {
      val controller = new SeriesDetailsController(getUnauthorisedSecurityComponents(), new GraphQLConfiguration(app.configuration))
      val seriesDetailsPage = controller.seriesDetails().apply(FakeRequest(GET, "/series"))
