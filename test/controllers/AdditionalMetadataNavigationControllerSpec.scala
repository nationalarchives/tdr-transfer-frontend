...
class AdditionalMetadataNavigationControllerSpec extends FrontEndTestHelper {
  ...
  "AdditionalMetadataNavigationController" should {
    ...
    "render the additional metadata file selection page, pagination buttons up to the total pages for large pages" in {
      ...
      setConsignmentTypeResponse(wiremockServer, "standard")
      setConsignmentDetailsResponse(wiremockServer, Option(parentFolder), parentFolderId = None)
      setConsignmentPaginatedFilesResponse(wiremockServer, parentFolder: String, folderId, fileId, totalPages = Some(7))
      ...
      fileSelectionPageAsString.contains(
        s"""<li class="govuk-pagination__item govuk-pagination__item--ellipses">&ctdot;</li>""") mustBe true
      fileSelectionPageAsString.contains(
        s"""class="govuk-button__tna-button-link" type="submit" data-module="govuk-button" role="link" aria-label="Page 6" value="6">""") mustBe false
      fileSelectionPageAsString.contains(
