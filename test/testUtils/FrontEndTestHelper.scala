...
import graphql.codegen.{GetCustomMetadata, GetDisplayProperties}
...
trait FrontEndTestHelper extends PlaySpec with MockitoSugar with Injecting with GuiceOneAppPerTest with BeforeAndAfterEach with TableDrivenPropertyChecks {
  ...
  private def getDisplayPropertiesDataObject: GetDisplayProperties.displayProperties.Data = {
    dp.Data(
      List(
  ...
  private def getCustomMetadataDataObject: GetCustomMetadata.customMetadata.Data = {
    cm.Data(
      List(
        cm.CustomMetadata(
          "ClosureType",
