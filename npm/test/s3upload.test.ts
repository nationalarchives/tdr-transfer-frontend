...
test("a single file upload returns the correct key", async () => {
  ...
  const mockUpload = mockLibStorageUpload()
  mockUpload.resolves({Key: "bob", $metadata: {httpStatusCode: 200}})
  const s3Upload = new S3Upload("http://localhost")
  // s3Upload.s3 = (s3Mock as unknown as S3Client)
